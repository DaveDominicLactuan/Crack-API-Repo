...
def preprocess_and_save_format(image_path):
    ...
    # which is more effective for varying lighting conditions to a black and white mask,
    #Why adaptive its more effective for varying lighting conditions,
    #which is common in real-world crack images, and give a custom threshold based on the 
    #lighting of the immeidate neighbor
    #Bluured the input image, maxValue is 255 is when it determined to be a crack turns it 
    #to pure white, uses gaussian for the adaptive method and inverse binary to make cracks white and background black, 
    #block size of 11 and C of 5 for fine-tuning
    ...
    #this function connects the dots/pixels and groups touching white pixels togther
...
#     """
#     Receives an image file via multipart/form-data, gives it a unique
#     filename, and stores it locally.
#     """
    
#     # Validation to ensure it's an image
#     if file.content_type not in ["image/jpeg", "image/png", "image/gif"]:
#         raise HTTPException(status_code=400, detail="File must be a JPEG, PNG, or GIF image.")

#     try:
#         # Generate a unique filename using UUID to prevent collisions
#         file_extension = os.path.splitext(file.filename)
        
#         # This will now work perfectly since uuid is imported
